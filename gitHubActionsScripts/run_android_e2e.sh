...
if [ -z "$0" ]; then
    echo "Usage: $0 <APP_ID>"
    echo "Usage: $1 <APP_PATH>"
    exit 1
fi

# Initialize APP_ID using the first argument
APP_ID=$1
# Initialize APP_PATH using the second argument
APP_PATH=$2

adb install $APP_PATH || (adb kill-server && adb start-server && adb devices && adb install $APP_PATH)
